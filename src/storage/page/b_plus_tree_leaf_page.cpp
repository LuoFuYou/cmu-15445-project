...
INDEX_TEMPLATE_ARGUMENTS
bool B_PLUS_TREE_LEAF_PAGE_TYPE::IsSafe(OpType op) {
  if (op == OpType::INSERT) {
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
void B_PLUS_TREE_LEAF_PAGE_TYPE::MoveHalfTo(BPlusTreeLeafPage *recipient, const KeyComparator &comparator) {
  for (int i = GetSize() / 2; i < GetSize(); i++) {
    recipient->Insert(array[i].first, array[i].second, comparator);
  }
...
bool B_PLUS_TREE_LEAF_PAGE_TYPE::Lookup(const KeyType &key, ValueType *value, const KeyComparator &comparator) const {
  ...
  while (l <= r) {
      int mid = (l + r) / 2;
      KeyType cur_key = array[mid].first;
      int res = comparator(key, cur_key);
      if (res == 0) {
...
void B_PLUS_TREE_LEAF_PAGE_TYPE::MoveFirstToEndOf(BPlusTreeLeafPage *recipient, const KeyComparator &comparator) {
  ...
  RemoveAndDeleteRecord(item.first, comparator);
